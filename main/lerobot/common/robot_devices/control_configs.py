...
class TeleoperateControlConfig(ControlConfig):
    ...
    # Enable keyboard control for follower arm
    keyboard_control: bool = False
    # Default mode when keyboard_control is enabled: "leader" or "keyboard"
    default_mode: str = "leader"


@ControlConfig.register_subclass("keyboard")
@dataclass
class KeyboardControlConfig(ControlConfig):
    # Limit the maximum frames per second. By default, no limit.
    fps: int | None = None
    teleop_time_s: float | None = None
    # Display all cameras on screen
